...
static const char blackboxHeader [] = "H Product:Blackbox flight data recorder by Nicholas Sherlock\n"
                                      "H Data version:2\n"
...
void finishBlackbox ( void ) {
  ...
  } else if ( blackboxState != BLACKBOX_STATE_DISABLED && blackboxState != BLACKBOX_STATE_STOPPED && blackboxState != BLACKBOX_STATE_SHUTTING_DOWN ) {
    /*
 ...
 */
static bool sendFieldDefinition ( char mainFrameChar, char deltaFrameChar, const void *fieldDefinitions, const void *secondFieldDefinition, int fieldCount, const uint8_t *conditions, const uint8_t *secondCondition ) {
...
static bool blackboxWriteSysinfo ( ) {
  ...
  switch ( xmitState.headerIndex ) {
    ...
    case 9:
      ...
      break;
    case 10:
      blackboxPrintfHeaderLine ( "vbatcellvoltage:%u,%u,%u", masterConfig.batteryConfig.vbatmincellvoltage, masterConfig.batteryConfig.vbatwarningcellvoltage, masterConfig.batteryConfig.vbatmaxcellvoltage );
      break;
    case 11:
...
void handleBlackbox ( void ) {
  ...
  switch ( blackboxState ) {
    ...
    case BLACKBOX_STATE_SEND_MAIN_FIELD_HEADER:
      ...
      if ( ! sendFieldDefinition ( 'I', 'P', blackboxMainFields, blackboxMainFields + 1, ARRAY_LENGTH ( blackboxMainFields ), &blackboxMainFields [ 0 ].condition, &blackboxMainFields [ 1 ].condition ) ) {
  #ifdef GPS
        if ( feature ( FEATURE_GPS ) ) {
    ...
    case BLACKBOX_STATE_SEND_GPS_H_HEADER:
      // On entry of this state, xmitState.headerIndex is 0 and xmitState.u.fieldIndex is -1
      if ( ! sendFieldDefinition ( 'H', 0, blackboxGpsHFields, blackboxGpsHFields + 1, ARRAY_LENGTH ( blackboxGpsHFields ), NULL, NULL ) ) {
        blackboxSetState ( BLACKBOX_STATE_SEND_GPS_G_HEADER );
      }
      break;
    case BLACKBOX_STATE_SEND_GPS_G_HEADER:
      // On entry of this state, xmitState.headerIndex is 0 and xmitState.u.fieldIndex is -1
      if ( ! sendFieldDefinition ( 'G', 0, blackboxGpsGFields, blackboxGpsGFields + 1, ARRAY_LENGTH ( blackboxGpsGFields ), &blackboxGpsGFields [ 0 ].condition, &blackboxGpsGFields [ 1 ].condition ) ) {
        blackboxSetState ( BLACKBOX_STATE_SEND_SLOW_HEADER );
      }
      break;
  #endif
    case BLACKBOX_STATE_SEND_SLOW_HEADER:
      ...
      if ( ! sendFieldDefinition ( 'S', 0, blackboxSlowFields, blackboxSlowFields + 1, ARRAY_LENGTH ( blackboxSlowFields ), NULL, NULL ) ) {
